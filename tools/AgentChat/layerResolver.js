...
function detectDomainConstraints(fullQueryNorm) {
  ...
  const hasSeaIce =
    fullQueryNorm.includes("seaice") || /\bsea\s+ice\b/.test(fullQueryNorm);
  return { hasSwot, hasSeaIce, hasFreeboard };
}

function detectSemanticRole(normalized) {
  if (
    /\b(prediction|predicted|predictive|forecast|forecasted|model output)\b/.test(
      normalized,
    )
  )
    return "prediction";
  if (
    /\bground\s+truth\b|\b(observed|observation|reference)\b/.test(normalized)
  )
    return "ground-truth";
  return null;
}

function detectLayerSemanticRole(layer, aliases = []) {
  // User-facing identity is authoritative. Internal ids often encode a
  // broader product family (for example, a ground-truth layer whose id
  // contains "forecast") and must not override an explicit display role.
  const displayRole = detectSemanticRole(
    normalizeLayerText(layer.displayName || layer.display || ""),
  );
  ...
  const aliasRoles = [
    ...new Set(
      aliases
        .map((alias) => detectSemanticRole(normalizeLayerText(alias)))
        .filter(Boolean),
    ),
  ];
  if (aliasRoles.length === 1) return aliasRoles[0];

  return detectSemanticRole(
    normalizeLayerText(layer.canonical || layer.name || ""),
  );
}
...
function buildCandidate(layer, queryNorm, queryTokens) {
  ...
  const displayNorm = normalizeLayerText(
    layer.displayName || layer.display || "",
  );
  ...
  const requestedRole = detectSemanticRole(queryNorm);
  const roleMatch =
    requestedRole != null &&
    detectLayerSemanticRole(layer, aliases) === requestedRole;
...
function isAmbiguous(top, second) {
  ...
  // A broad semantic role such as "prediction" or "ground truth" is useful
  // grounding, but it must not silently choose among multiple role-matching
  // mission layers. Ask for clarification unless stronger tokens/exact names
  // distinguish the candidates.
  if (
    top.roleMatch &&
    second.roleMatch &&
    !top.exactDisplayOrName &&
    !top.exactAlias &&
    !top.containsAllTokens
  )
    return true;
  ...
  if (top.tier <= 1 && top.fuzzy < 0.72) {
    ...
    if (second.tier <= 1 && Math.abs(top.fuzzy - second.fuzzy) < 0.08)
      return true;
    return false;
  }
...
export function resolveLayerSelection({
  ...
  const ranked = layers
    .filter(
      (layer) =>
        String(layer?.type || layer?.config?.type || "").toLowerCase() !==
        "header",
    )
    .map((layer) => buildCandidate(layer, scoringQueryNorm, queryTokens))
    .filter(
      (candidate) =>
        // An explicit layer argument is stronger grounding than broad
        // domain words in the surrounding request. For example, a
        // resolved config name such as "SFNO Prediction Daily" must
        // remain executable for a "sea ice forecast" request even if
        // its display name does not literally contain "sea ice".
        candidate.exactDisplayOrName ||
  ...
  // A single merely-best candidate is not a meaningful match. Returning
  // no match here prevents arbitrary layers from being selected for
  // unrelated/gibberish names just because they won a fuzzy ranking.
  if (top.tier <= 1 && top.fuzzy < 0.65) {
    return { match: null, candidates: [] };
  }
  ...
  if (ambiguous) {
    ...
    const secondName = normalizeLayerText(
      second.layer.displayName ||
        second.layer.display ||
        second.layer.name ||
        "",
    );
    if (topName && topName === secondName) {
      ...
      const bestAlias =
        pick.aliases[0] || pick.layer.displayName || pick.layer.name;
      return {
        match: {
          original: requestedName,
          resolved:
            pick.layer.displayName || pick.layer.display || pick.layer.name,
          alias: bestAlias,
          score: pick.fuzzy,
          exact: pick.tier >= 3,
